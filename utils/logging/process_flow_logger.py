...
class ProcessFlowLogger:
    ...
    def _print_separator(self, char: str = "═", length: int = 60) -> None:
        ...
        print(self._colorize(char * length, 'blue'))
    ...
    def start_experiment(self, experiment_id: str, config: Dict[str, Any]) -> None:
        ...
        if self._should_log(VerbosityLevel.MINIMAL):
            ...
            print(self._colorize("🧪 FROHLICH EXPERIMENT STARTING", 'bold'))
            self._print_separator()
            
        if self._should_log(VerbosityLevel.STANDARD):
            print(f"Experiment ID: {self._colorize(experiment_id[:8], 'blue')}")
            print(f"Participants: {self._colorize(str(len(config.get('agents', []))), 'green')}")
            print(f"Language: {self._colorize(config.get('language', 'English'), 'yellow')}")
            print(f"Max Phase 2 rounds: {self._colorize(str(config.get('phase2_rounds', 'N/A')), 'yellow')}")
            
        if self._should_log(VerbosityLevel.DETAILED):
            print(f"Configuration file: {config.get('config_file', 'N/A')}")
    ...
    def initialize_agents(self, agent_configs: List[Dict[str, Any]]) -> None:
        ...
        if self._should_log(VerbosityLevel.STANDARD):
            print(self._colorize("🤖 Initializing Agents...", 'bold'))
    ...
    def agent_initialized(self, agent_name: str, model: str, duration: float) -> None:
        ...
        if self._should_log(VerbosityLevel.STANDARD):
            ...
            print(f"  ✅ {self._colorize(agent_name, 'green')} ({model_display}){duration_str}")
    ...
    def start_phase1(self, participant_count: int) -> None:
        ...
        if self._should_log(VerbosityLevel.MINIMAL):
            print(self._colorize("📖 PHASE 1: Individual Familiarization", 'bold'))
            
        if self._should_log(VerbosityLevel.STANDARD):
            print(self._colorize("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", 'blue'))
            print(f"Running parallel familiarization for {participant_count} participants...")
            print()
    ...
    def phase1_agent_progress(self, agent_name: str, step: str, progress: float) -> None:
        ...
        if self._should_log(VerbosityLevel.DETAILED):
            ...
            print(f"  {agent_name}: {step} {progress_bar}")
    
    def phase1_completed(self, results_summary: List[Dict[str, Any]], duration: float) -> None:
        """Log Phase 1 completion."""
        if self._should_log(VerbosityLevel.STANDARD):
            print(f"\n🎯 {self._colorize('Phase 1 Complete', 'green')} - Duration: {self._format_duration(duration)}")
            
        if self._should_log(VerbosityLevel.DETAILED):
            for result in results_summary:
                name = result.get('name', 'Unknown')
                earnings = result.get('earnings', 0.0)
                print(f"  {name}: ${earnings:.2f} earned")
    ...
    def start_phase2(self, max_rounds: int) -> None:
        ...
        if self._should_log(VerbosityLevel.MINIMAL):
            print(self._colorize("💬 PHASE 2: Group Discussion", 'bold'))
            
        if self._should_log(VerbosityLevel.STANDARD):
            print(self._colorize("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", 'blue'))
            print(f"Beginning group discussion (max {max_rounds} rounds)...")
            print()
    ...
    def phase2_round_start(self, round_num: int, max_rounds: int, speaking_order: List[str]) -> None:
        ...
        if self._should_log(VerbosityLevel.STANDARD):
            ...
            print(f"🗣️  {self._colorize(f'Round {round_num}{progress_indicator}', 'bold')}")
            
            if self._should_log(VerbosityLevel.DETAILED):
                speaking_list = " → ".join(speaking_order)
                print(f"   Speaking order: {speaking_list}")
    ...
    def phase2_agent_speaking(self, agent_name: str, round_num: int) -> None:
        ...
        if self._should_log(VerbosityLevel.DETAILED):
            print(f"   💭 {self._colorize(agent_name, 'yellow')} is thinking...")
    ...
    def phase2_agent_response(self, agent_name: str, message_length: int, response_time: float) -> None:
        ...
        if self._should_log(VerbosityLevel.DETAILED):
            ...
            print(f"   ✅ {agent_name} responded ({message_length} chars){timing}")
    ...
    def phase2_voting_initiated(self, round_num: int) -> None:
        ...
        if self._should_log(VerbosityLevel.STANDARD):
            print(f"🗳️  {self._colorize('Voting initiated', 'bold')} (Round {round_num})")
    ...
    def phase2_voting_result(self, consensus_reached: bool, chosen_principle: Optional[str], 
        ...
        if consensus_reached:
            if self._should_log(VerbosityLevel.MINIMAL):
                ...
                print(f"🎉 {self._colorize('Consensus Reached!', 'green')} Principle: {principle_display}{constraint_text}")
        else:
            if self._should_log(VerbosityLevel.STANDARD):
                print(f"⏳ No consensus yet - continuing discussion...")
        print()
    ...
    def phase2_completed(self, consensus_reached: bool, final_round: int, duration: float, 
        ...
        if self._should_log(VerbosityLevel.STANDARD):
            ...
            print(f"🏁 {self._colorize(f'Phase 2 Complete {status}', 'green')} - Duration: {self._format_duration(duration)}")
            print(f"   Rounds conducted: {final_round}")
    ...
    def experiment_completed(self, experiment_summary: Dict[str, Any]) -> None:
        ...
        if self._should_log(VerbosityLevel.MINIMAL):
            self._print_separator()
            print(self._colorize("✅ EXPERIMENT COMPLETED", 'bold'))
            self._print_separator()
            
        if self._should_log(VerbosityLevel.STANDARD):
            print(f"Total Duration: {self._colorize(self._format_duration(total_duration), 'green')}")
        ...
        if self._should_log(VerbosityLevel.DETAILED):
            ...
            if errors > 0:
                print(f"Errors: {errors} (all recovered)")
            else:
                print("Errors: None")
    ...
    def log_warning(self, message: str, context: Optional[str] = None) -> None:
        ...
        if self._should_log(VerbosityLevel.STANDARD):
            ...
            print(f"⚠️  {self._colorize(message, 'yellow')}{context_str}")
    
    def log_error(self, message: str, context: Optional[str] = None) -> None:
        """Log an error message."""
        if self._should_log(VerbosityLevel.MINIMAL):
            context_str = f" ({context})" if context else ""
            print(f"❌ {self._colorize(message, 'red')}{context_str}")
    
    def log_recovery(self, message: str) -> None:
        """Log error recovery."""
        if self._should_log(VerbosityLevel.STANDARD):
            print(f"🔄 {self._colorize(message, 'blue')}")
    ...
    def log_debug(self, message: str) -> None:
        ...
        if self._should_log(VerbosityLevel.DEBUG):
            print(f"{self._colorize('[DEBUG]', 'gray')} {message}")
    
    def log_technical(self, message: str) -> None:
        """Log technical details (only in DEBUG mode)."""
        if self._should_log(VerbosityLevel.DEBUG):
            print(f"{self._colorize('[TECH]', 'gray')} {message}")
