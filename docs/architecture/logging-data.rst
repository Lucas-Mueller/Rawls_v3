...
The system produces three complementary outputs on every run:
...
3. **Structured Summary** – ``utils.logging.result_summary.build_experiment_summary`` emits ``experiment_results_<timestamp>_summary.json`` with high-level metrics such as runtime, consensus status, and earnings by participant.
...
- Notifications about where results and summaries were saved

Integration Tips
----------------

- **Archival**: Store both JSON files together; the summary is intentionally named after the primary log to simplify grouping.
- **Automation**: Downstream scripts can glob for ``*_summary.json`` to build reports without parsing full agent trails.
- **Debugging**: Use the agent log when you need the full transcript of decisions or memory updates; use the summary to sanity-check outcomes quickly.
