...
Contract tests live under `tests/snapshots/contracts/` and assert on golden artefacts
produced by live experiment runs. They intentionally keep assertions stable, so
updates must be deliberate.

## When to Run
- Local sanity check: `pytest tests/snapshots/contracts`
...
2. Run the affected contract test(s) with `pytest -k <test_name> tests/snapshots/contracts`.
3. Inspect the generated diff (usually under `tests/snapshots/contracts/fixtures` or
...
- [ ] Tests pass locally (`pytest tests/snapshots/contracts`)
