...
**Remove and Replace, Don't Accumulate**: When introducing new functionality that supersedes existing code, always remove the old implementation to reduce complexity. This applies to:
- Methods that are replaced by better implementations
- Deprecated patterns or approaches  
- Legacy code paths that are no longer needed
- Redundant functionality

**Utility Agent First**: Prefer intelligent utility agent-based processing over hardcoded regex patterns, especially for:
- Text parsing and constraint extraction
